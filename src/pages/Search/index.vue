...
<script>
  ...
  export default {
    ...
    methods: {
      //像服务器发请求获取search模块的数据（根据参数不同返回不同的数据进行展示）
      //把这次请求封装成一个函数，当你需要在调用的时候调用即可
      getData(){
        this.$store.dispatch('getSearchList',this.searchParams)
      },
      removeCategoryName(){
        //带给服务器的参数都是可有可无的，如果属性为空字符串还是会带给服务器，所以置为undefined就不会带了
      ...
      //排序操作
      changeOrder(flag){
        //flag是一个代表用户点击的是综合还是价格
        let order=this.searchParams.order
        if(order.split(':')[0]=='1'){
          if(flag=='1'){
            //如果原先是综合点击的还是综合则改变升降序
            order=order.includes('asc')?order.replace('asc','desc'):order.replace('desc','asc')
          }else if(flag=='2'){
            order=order.replace('1','2')
          }
        }else{
          if(flag=='1'){
            order=order.replace('2','1')
          }else if(flag=='2'){
            order=order.includes('asc')?order.replace('asc','desc'):order.replace('desc','asc')
          }
        }
