...
export default {
  ...
  methods: {
    ...
    async addShopcar(){
      //1派发actions,通知服务器,得到一个promise结果
      //2服务器存储成功进行路由跳转并且带参，失败给用户进行提示
      try {
        await this.$store.dispatch('addOrUpdateShopCart',{skuId:this.$route.params.skuId,skuNum:this.skuNum})
        //3再进行路由跳转
        //4在路由跳转的时候还要将产品信息带给下一级的组件
        //下面这种方法传递参数可以，但是url显示会出现长串的不清楚含义的字符串
        // this.$router.push({name:'addcartsuccess',query:{skuInfo:this.skuInfo,skuNum:this.skuNum}})
        //这里复杂的skuInfo使用本地存储,而简单的数据skuNum使用query传参
